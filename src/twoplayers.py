...
class TwoPlayerAgent:
    ...
    def calculate_trans_probs_real(self):
        trans_probs_cur = np.zeros(shape=(self.n_states, self.n_actions, self.n_states), dtype=np.float32)
        ...
        for i in range(self.n_states):
            ...
            trans_probs_cur[i, 0, (real_value_0 + int(x)*2) ] = 0.5
            trans_probs_cur[i, 1, (real_value_0 + int(x) * 2 + 1)] = 0.5
            trans_probs_cur[i, 0, (real_value_1 + int(x) * 2)] = 0.5
            trans_probs_cur[i, 1, (real_value_1 + int(x) * 2 + 1)] = 0.5
...
class Copyer(TwoPlayerAgent):
    ...
    def calculate_rewards(self):
        ...
        for i in range(self.n_states):
            binary_number = '{0:04b}'.format(i)
            if (binary_number[0] == binary_number[3]):
                rewards_cur[i] = 1
        return rewards_cur

class Defecter(TwoPlayerAgent):
    def __init__(self):
        TwoPlayerAgent.__init__(self)
        self._rewards = self.calculate_rewards()
    ...
    def calculate_rewards(self):
        ...
        for i in range(self.n_states):
            binary_number = '{0:04b}'.format(i)
            if (binary_number[2] == 0 and binary_number[3] == 0):
                rewards_cur[i] = 1
        return rewards_cur
