...
# Computing Q values via iterating V_values until we have the maximum possible (at this point the new delta would be lower than the old delta => check this)
...
# calculates best policy for current environment (at start when no Pi is given, it just generates a random Pi)
def policy_iteration(env, gamma, pi=None):
    if pi is None:
        # array size "env.n_states" with given action they should take (so random integer from 0 to n_actions-1, since action start at 0 anyways)
        pi = np.random.randint(env.n_actions, size=env.n_states)
        #print("RANDOM POLICY IS: " + str(pi))
    ...
    while True:
        ...
        #print(__compute_q_with_v(env, state_values, gamma))
        pi = np.argmax(__compute_q_with_v(env, state_values, gamma), axis=1)
        if np.all(old_pi == pi):
            return pi
        else:
            #WITH LOW GAMMA THIS OCCURS, LOCAL MINIMUM, FOR NOW JUST RETURN, IS PROBABLY WHEN 2 POLICIES HAVE SAME REWARD FUNCTION WHICH WITH LOWER GAMMA CAN OCCUR
            n_iter += 1
            if n_iter > 1000:
                print('n_iter: ', n_iter)
...
# This is just computing the Q value based on current V-values (this function is used in "compute_q_via_dp" function)
def __compute_q_s_with_v(env, s, state_values, gamma):
    ...
    #x = np.sum(env.trans_probs[s] * state_values, axis=-1)
    assert q_values_s.shape == (env.n_actions,), 'Invalid shape computed'
    return q_values_s

# Function I wrote myself, when Q-values are given at the end (or intermediate steps) we want a (current) policy to use
def compute_policy_for_q(q_values):
